...
from utils.tp_utils import tp_path_exists
...
class ConvNetLayer(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        self.non_local=NonlocalInteraction(self.irreps_in[AtomicDataDict.NODE_FEATURES_KEY].dim,1,1,1).float()
...
#        self.linear_non_local= torch.nn.Linear(32,352)
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        old_x_non = self.non_local(old_x,num_batch=int(data['batch'][-1]+1),batch_seg=data['batch'])
        # run convolution
        data = self.conv(data)
        # do nonlinearity
        ...
        if self.resnet:
            print('resnet_true')
            data[AtomicDataDict.NODE_FEATURES_KEY] = (
                old_x + old_x_non + data[AtomicDataDict.NODE_FEATURES_KEY]
            )
        return data
