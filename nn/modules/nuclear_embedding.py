...
class NuclearEmbedding(nn.Module):
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        Z = data[AtomicDataDict.ATOM_TYPE_KEY].squeeze(-1)      
        ...
        if self.embedding.device.type == "cpu":  # indexing is faster on CPUs
            data[AtomicDataDict.NODE_FEATURES_KEY] = self.embedding[Z]

        else:  # gathering is faster on GPUs
            data[AtomicDataDict.NODE_FEATURES_KEY] = torch.gather(
