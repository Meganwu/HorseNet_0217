...
class InteractionBlock(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        self.non_local=NonlocalInteraction(32,1,1,1).float()

        self.linear_non_local= torch.nn.Linear(32,352)
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        x_local = data[AtomicDataDict.NODE_FEATURES_KEY]
        x_non = data[AtomicDataDict.NODE_FEATURES_KEY] 
        edge_src = data[AtomicDataDict.EDGE_INDEX_KEY][1]
        edge_dst = data[AtomicDataDict.EDGE_INDEX_KEY][0]
#        print("x_non:", x_non.shape)
#        print("x_new:",x_local.shape)

        x_non_local=self.non_local(x_non,num_batch=int(data['batch'][-1]+1),batch_seg=data['batch'])
#        x_local=x_local+x_non_local 
 #       x_non_local=self.linear_non_local(x_non_local)
#        print("x_non_local:",x_non_local.shape)
        if self.sc is not None:
            sc = self.sc(x_local, data[AtomicDataDict.NODE_ATTRS_KEY])

        x_local = self.linear_1(x_local)
        edge_features = self.tp(
            x_local[edge_src], data[AtomicDataDict.EDGE_ATTRS_KEY], weight
        )

        x_local = scatter(edge_features, edge_dst, dim=0, dim_size=len(x_local))
        ...
        if avg_num_neigh is not None:
            x_local = x_local.div(avg_num_neigh**0.5)

        x_local = self.linear_2(x_local)
        x_non_local=self.linear_non_local(x_non_local)
        print("x_non_local:", x_non_local.shape)
        print("x_local:",x_local.shape)
        x_total = x_local + x_non_local
        if self.sc is not None:
            x_total = x_total + sc 

        data[AtomicDataDict.NODE_FEATURES_KEY] = x_total
        return data
