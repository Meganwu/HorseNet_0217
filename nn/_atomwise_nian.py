...
class AtomwiseLinearFinal(GraphModuleMixin, torch.nn.Module):
    def __init__(
        self,
        field: str = AtomicDataDict.NODE_FEATURES_KEY,
        out_field: Optional[str] = None,
        out_field_1: str = AtomicDataDict.PER_ATOM_ENERGY_KEY,
        out_field_2: str = AtomicDataDict.PER_ATOM_CHARGE_KEY,
        irreps_in=None,
        irreps_out=None,
    ):
        super().__init__()
        self.field = field
        out_field = out_field if out_field is not None else field
        self.out_field_1 = out_field_1
        self.out_field_2 = out_field_2
        if irreps_out is None:
            irreps_out = irreps_in[field]
        ...
        self._init_irreps(
            irreps_in=irreps_in,
            required_irreps_in=[field],
            irreps_out={out_field: '2x0e', out_field_1: '1x0e', out_field_2: '1x0e'},
        )
        self.linear = Linear(
            irreps_in=self.irreps_in[field], irreps_out=self.irreps_out[out_field]
        )
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        data[self.out_field_1] = data[self.field][:,0].unsqueeze(1)
        data[self.out_field_2] = data[self.field][:,1].unsqueeze(1)
        return data
