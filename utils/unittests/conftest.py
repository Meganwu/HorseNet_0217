...
from utils.test import set_irreps_debug
from data import AtomicData, ASEDataset
from data.transforms import TypeMapper
from utils.torch_geometric import Batch
from utils._global_options import _set_global_options
from utils.misc import dtype_from_name
...
@pytest.fixture(scope="session")
def dataset(molecules, temp_data, float_tolerance):
...
@pytest.fixture(scope="session")
def atomic_batch(dataset):
    return Batch.from_data_list([dataset[0], dataset[1]])
