...
class AtomicData(Data):
    ...
    def from_points(
        ...
        **kwargs,
        ...
        if atomic_charges is not None:
           kwargs[AtomicDataDict.PER_ATOM_CHARGE_KEY] = torch.as_tensor(atomic_charges,dtype=torch.get_default_dtype()).view(-1)
           
        if total_charge is not None:
            kwargs[AtomicDataDict.TOTAL_CHARGE_KEY] = torch.as_tensor(total_charge,dtype=torch.get_default_dtype()).long() 
    ...
    def from_ase(
        ...
        **kwargs,
        ...
        km = {
            "atomic_charges":  AtomicDataDict.PER_ATOM_CHARGE_KEY,
        ...
        atomic_charges=kwargs.pop("atomic_charges",atoms.get_initial_charges())
        total_charge=kwargs.pop("total_charge",np.zeros([1]))
