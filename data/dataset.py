        ...
        for ifield, field in enumerate(fields):
            if callable(field):
                ...
                if field not in selectors:
                    ...
                    raise RuntimeError(
                        f"Only per-node and per-graph fields can have statistics computed; `{field}` has not been registered as either. If it is per-node or per-graph, please register it as such using `data.register_fields`"
                    )
